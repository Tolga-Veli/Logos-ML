...
public:
  SGD(std::vector<core::Parameter<T> *> params, T learning_rate,
    ...
    if (m_Momentum > T{0}) {
      ...
      for (auto *p : m_Params)
        m_Velocity.emplace_back(p->data.shape(), T{0});
    }
  }
...
private:
  std::vector<core::Parameter<T> *> m_Params;
  std::vector<ml::core::Tensor<T>> m_Velocity;
