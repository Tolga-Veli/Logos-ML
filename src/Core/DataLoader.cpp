...
DataLoader::DataLoader(Tensor images, Tensor labels, std::size_t batch_size, bool shuffle)
  ...
  if (m_Images.rank() != 2 || m_Labels.rank() != 1)
    throw ShapeError("DataLoader: expects images [N, features] and labels [N]");
  if (!std::isfinite(batch_size))
    throw ShapeError("DataLoader: batch_size must be positive");

  m_Count = m_Images.shape()[0];
  m_Features = m_Images.shape()[1];

  if (m_Labels.shape()[0] != m_Count)
    throw ShapeError("DataLoader: images/labels count mismatch");
  if (!m_Images.is_contiguous() || !m_Labels.is_contiguous())
    throw ShapeError("DataLoader: requires contiguous input tensors");

  m_Indices.resize(m_Count);
  std::iota(m_Indices.begin(), m_Indices.end(), 0uz);
}
